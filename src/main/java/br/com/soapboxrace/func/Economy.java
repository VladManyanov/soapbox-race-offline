...
public class Economy {
	...
	public boolean transCurrency(boolean remove) {
		try {
			...
			Functions.log("  || -> Ваш счёт" + (type == IGC ? " IGC " : " Boost ") + "равен " + String.valueOf(newAmount) + ".");
			return true;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Economy(String catalogName, String productId, Boolean type2) {
		try {
			if (catalogName == null || productId == null) {
				Functions.log("  || -> Модуль экономики не поддерживает этот объект. Счёт не будет изменён.");
				return;
			}
			Functions.log("  || -> Запуск модуля экономики.");
			if (type2) {
				...
				Functions.log("  || -> Покупка комплекта " + productTitle + " за " + String.valueOf(amount) + (type == IGC ? " IGC" : " Boost") + ".");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void AddCommerce(String catalogName, String productId) {
		try {
			...
			String productTitle = catalog.getElementsByTagName("ProductTitle").item(catID).getTextContent().isEmpty() ? "Unknown Item"
					: catalog.getElementsByTagName("ProductTitle").item(catID).getTextContent();
			Functions.log("    || -> Добавлен комплект " + productTitle + " за " + String.valueOf(amount) + (type == IGC ? " IGC" : " Boost") + " в список покупок.");
			Functions.log("  || -> Полная цена: " + String.valueOf(totalPrice[0]) + " IGC и " + String.valueOf(totalPrice[1]) + " Boost");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public boolean DoCommerce() {
		if (totalPrice[0] > 0) {
			Functions.log("  || -> Запрос покупки за " + String.valueOf(totalPrice[0]) + " IGC.");
			type = IGC;
			amount = totalPrice[0];
		...
		if (totalPrice[1] > 0) {
			Functions.log("  || -> Запрос покупки за " + String.valueOf(totalPrice[1]) + " Boost.");
			type = Boost;
			amount = totalPrice[1];
			if (!transCurrency(true))
				return false;
	...
	public void AddPartToSell(String price) {
		...
		Functions.log("    || -> Добавлена одна деталь за " + String.valueOf((int) Math.round(Double.valueOf(price))) + "IGC.");
	}

	public void SellParts() {
		if (totalPrice[0] != 0) {
			Functions.log("  || -> " + String.valueOf(totalPrice[0]) + "IGC добавлено на ваш счёт.");
			type = IGC;
			amount = totalPrice[0];
			transCurrency(false);
		}
